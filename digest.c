...
void finalize_digest(digest_ctx *context) {
  memset(context->block + context->block_len, 0,
         DIGEST_BLOCK_SIZE - context->block_len);
...
int main(int argc, char *argv[]) {
  ...
  if (!(strcmp(argv[1], "-md5"))) {
    ...
    digest_hash(decoded_input, decoded_len, hash, md5_block_operate,
                md5_finalize);
    ...
    digest_hash(decoded_input, decoded_len, hash, sha1_block_operate,
                sha1_finalize);
  } else {
