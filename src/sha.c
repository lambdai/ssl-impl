...
unsigned int shr(unsigned int x, unsigned int n) { return x >> n; }
...
void sha256_block_operate(const unsigned char *block, unsigned int hash[8]) {
  unsigned int W[64];
  ...
  for (t = 0; t < 64; t++) {
    if (t <= 15) {
      ...
      W[t] = sigma_shr(W[t - 2], 1) + W[t - 7] + sigma_shr(W[t - 15], 0) +
             W[t - 16];
    }
  }
...
unsigned int sha1_initial_hash[] = {0x01234567, 0x89abcdef, 0xfedcba98,
                                    0x76543210, 0xf0e1d2c3};
...
void new_sha1_digest(digest_ctx *context) {
  ...
  context->block_len = 0;
  context->hash =
      (unsigned int *)malloc(context->hash_len * sizeof(unsigned int));
...
void new_sha256_digest(digest_ctx *context) {
  ...
  context->hash =
      (unsigned int *)malloc(context->hash_len * sizeof(unsigned int));
  memcpy(context->hash, sha256_initial_hash,
         context->hash_len * sizeof(unsigned int));
  memset(context->block, '\0', DIGEST_BLOCK_SIZE);
