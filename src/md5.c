...
#define ROUND(F, a, b, c, d, k, s, i)                                          \
  a = (a + F(b, c, d) + x[k] + (unsigned int)(BASE_T * fabs(sin((double)i)))); \
  a = (a << s) | (a >> (32 - s));                                              \
  a += b;

unsigned int md5_initial_hash[] = {0x67452301, 0xefcdab89, 0x98badcfe,
                                   0x10325476};
...
void md5_block_operate(const unsigned char *input,
  ...
  for (j = 0; j < 16; j++) {
    x[j] = input[(j * 4) + 3] << 24 | input[(j * 4) + 2] << 16 |
           input[(j * 4) + 1] << 8 | input[(j * 4)];
  }
...
int md5_hash(const unsigned char *input, int len,
  ...
  // XXX should verify that len < 2^64, but since len is only 32 bits, this
  // won't be a problem.
