 ...
 * false otherwise. This isn't shown in the book.
 */
int validate_node(struct asn1struct *source, int expected_tag,
                  int expected_children, const char *desc) {
...
static const unsigned char OID_md5WithRSA[] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                               0x0D, 0x01, 0x01, 0x04};
static const unsigned char OID_sha1WithRSA[] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                                0x0D, 0x01, 0x01, 0x05};
static const unsigned char OID_sha1WithDSA[] = {0x2A, 0x86, 0x48, 0xCE,
                                                0x38, 0x04, 0x03};
...
static int parse_algorithm_identifier(signatureAlgorithmIdentifier *target,
  ...
  if (!validate_node(oid, ASN1_OBJECT_IDENTIFIER, 0,
...
int parse_name(name *target, struct asn1struct *source) {
  ...
  while (typeValuePair) {
    ...
    if (!validate_node(typeValuePairSequence, ASN1_SEQUENCE, 2,
                       "tag value pair in name")) {
      return 2;
    }

    type = (struct asn1struct *)typeValuePairSequence->children;

    if (!validate_node(type, ASN1_OBJECT_IDENTIFIER, 0,
                       "tag value pair in name type")) {
      return 3;
    }
    ...
    if (!(value->tag == ASN1_PRINTABLE_STRING ||
          value->tag == ASN1_TELETEX_STRING || value->tag == ASN1_IA5_STRING ||
          value->tag == ASN1_UTF8_STRING)) {
      fprintf(stderr,
              "Error parsing tag value pair in name, expected a string tag, "
              "got a %d\n",
              value->tag);
      return 4;
    }
...
static int parse_validity(validity_period *target, struct asn1struct *source) {
  ...
  if ((not_before->tag != ASN1_UTC_TIME) &&
      (not_before->tag != ASN1_GENERALIZED_TIME)) {
  ...
  if ((not_after->tag != ASN1_UTC_TIME) &&
      (not_after->tag != ASN1_GENERALIZED_TIME)) {
  ...
  if (sscanf((char *)not_before->data, "%2d%2d%2d%2d%2d%2d",
             &not_before_tm.tm_year, &not_before_tm.tm_mon,
             &not_before_tm.tm_mday, &not_before_tm.tm_hour,
             &not_before_tm.tm_min, &not_before_tm.tm_sec) < 6) {
  ...
  if (sscanf((char *)not_after->data, "%2d%2d%2d%2d%2d%2d",
             ...
             &not_after_tm.tm_hour, &not_after_tm.tm_min,
             &not_after_tm.tm_sec) < 6) {
...
static const unsigned char OID_RSA[] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                        0x0D, 0x01, 0x01, 0x01};
static const unsigned char OID_DSA[] = {0x2A, 0x86, 0x48, 0xCE,
                                        0x38, 0x04, 0x01};

static int parse_dsa_params(public_key_info *target,
                            struct asn1struct *source) {
  struct asn1struct *p;
  struct asn1struct *q;
  struct asn1struct *g;
...
static int parse_public_key_info(public_key_info *target,
  ...
  if (asn1parse(public_key->data + 1, public_key->length - 1,
                &public_key_value)) {
    fprintf(stderr,
  ...
  if (!memcmp(oid->data, &OID_RSA, sizeof(OID_RSA))) {
    ...
    if (!validate_node(&public_key_value, ASN1_SEQUENCE, 2,
                       "RSA public key value")) {
      return 6;
    }

    parse_huge(target->rsa_public_key.modulus, public_key_value.children);
    parse_huge(target->rsa_public_key.exponent,
               public_key_value.children->next);
    ...
    if (!validate_node(&public_key_value, ASN1_INTEGER, 0,
...
int asn1_get_bit(const int length, const unsigned char *bit_string,
                 const int bit) {
  if (bit > ((length - 1) * 8)) {
    return 0;
  } else {
...
static int parse_extension(x509_certificate *certificate,
  ...
  if (!memcmp(oid->data, OID_keyUsage, oid->length)) {
    ...
    if (asn1_get_bit(key_usage_bit_string.length, key_usage_bit_string.data,
                     BIT_CERT_SIGNER)) {
...
int validate_certificate_rsa(signed_x509_certificate *certificate,
  ...
  pkcs7_signature_len = rsa_decrypt(certificate->rsa_signature_value.rep,
                                    certificate->rsa_signature_value.size,
                                    &pkcs7_signature_decrypted, public_key);
...
int validate_certificate_dsa(signed_x509_certificate *certificate) {
  return dsa_verify(
      ...
      certificate->hash, certificate->hash_len * 4,
...
static void output_x500_name(name *x500_name) {
  printf(
      ...
      (x500_name->idAtOrganizationalUnitName
           ? x500_name->idAtOrganizationalUnitName
           : "?"),
      (x500_name->idAtCommonName ? x500_name->idAtCommonName : "?"));
}

static void print_huge(huge *h) { show_hex(h->rep, h->size); }
...
void display_x509_certificate(signed_x509_certificate *certificate) {
  ...
  printf("not before: %s",
         asctime(gmtime(&certificate->tbsCertificate.validity.notBefore)));
  printf("not after: %s",
...
int main(int argc, char *argv[]) {
  ...
  init_x509_certificate(&certificate);
  if (!(error_code =
            parse_x509_certificate(buffer, buffer_size, &certificate))) {
